...
class Uploader {
    ...
    var $fileFormat = array('gif', 'jpg', 'doc', 'png', 'jpeg', 'bmp', 'application/octet-stream'); // 文件格式&MIME限定
    ...
    function __construct($savePath = '', $fileFormat='', $maxSize = 0) {
    ...
    function copyfile($fileArray) {
        ...
        // 返回信息
        $this->returninfo['oldname'] = $fileArray['name'];
        $this->returninfo['newName'] = $this->saveName;
        $this->returninfo['fileName'] = $this->relativePath . $this->saveName;
    ...
    function setSavepath($savePath = '') {
        ...
        $this->relativePath = substr(str_replace("\\", "/", $savePath), -1) == "/" ? $savePath : $savePath . "/";
        $tureDir = ROOT_DIR . $this->relativePath;
        $this->savePath = substr(str_replace("\\", "/", $tureDir), -1) == "/" ? $tureDir : $tureDir . "/";
    ...
    function setSavename() {
        $this->saveName = date('YmdHis') . rand(10, 99) . '.' . $this->ext;
    }
